...
@author: kiran
basi script to holduseful ansatz function circuits, returns handels to circuits 
...
def GHZ_3qubits_6_params(swaps = 0):
    if swaps == 0:
        return (_GHZ_3qubits_6_params_x0, 6, 3, 
                np.array([3., 3., 2., 3., 3., 1.]) * np.pi/2)
    elif swaps == 1:
        return (_GHZ_3qubits_6_params_x1, 6, 3, 
                np.array([3., 3., 2., 3., 3., 1.]) * np.pi/2)
    else:
        assert False, "map not yet implimented"
...
def _GraphCycl_6qubits_6params_inefficient(params, barriers = False):        
    """ Ansatz for which an ideal solution exist with INEFFICIENT cnot implimentation"""
    logical_qubits = qk.QuantumRegister(6, 'logicals')
    c = qk.QuantumCircuit(logical_qubits)
    c.ry(params[0],0)
    c.ry(params[1],1)
